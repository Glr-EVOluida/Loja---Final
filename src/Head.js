...
export class Head extends React.Component {
  ...
  getClientes = () => {
    ...
    if (user === "" || pass === "") {
      ...
      fetch(
        `http://localhost:4000/show?table=clientes&where=email="${user}" AND senha="${passCrypto}"`
      )
        .then(response => response.json())
        .then(response =>
          response.data.length === 0
            ...
            : this.setState({ cliente: response.data }, () =>
                response.data.map(this.Verificacao)
              )
        )
          ...
          <div className="dropdown-content">
            <div className="form-group">
              <div className="center">
                ...
                <a
                  className="menuUser"
                  onClick={() => this.props.handleChangePage("perfil")}
                >
                  <i className="fas fa-user" /> Meu Perfil
                </a>
                <br />
                {cliente[0].admin === 1 ? (
                  <a
                    className="menuUser"
                    onClick={() => this.props.handleAdmin(true)}
                  >
                    <i className="fas fa-user-tie" /> Admin
                  </a>
                ) : (
                  ""
                )}
            ...
            >
              <Modal.Body className="center">
                ...
                <Button
                  ...
                  type="submit"
                  onClick={() =>
                    this.setState({ login: false, user: "", pass: "" }, () => {
                      cookie.remove("usuario", { path: "/" });
                      this.props.handleChangePage("");
                    })
                  }
  ...
  Login = () => {
    return (
      <div>
        <div className="logPc" id="dropdown">
          <div className="dropdown">
            <i className="fas fa-user-circle icoL" />
          </div>
          <div className="dropdown-content" style={{ zIndex: 29 }}>
            <div className="form-group">
              ...
              <input
                type="email"
                onKeyDown={this.handleKeyPressLogin}
  ...
  render() {
    ...
    const getItmes = nome =>
      nome ? matchSorter(items, nome, { keys: ["nome"] }) : items;

    return (
      <div className="head">
        <div className="container">
          <div className="col-md-1 col-xs-12">
            <a href="#!" onClick={() => this.props.handleChangePage("")}>
              <img
                className="logo"
                src={BaraTudo}
                alt="BaraTudo"
                    ...
                    <ul className="list-ul" {...getMenuProps()}>
                      {isOpen ? (
                        getItmes(inputValue) == "" ? (
                          <li>
                            {" "}
                            Nenhum produto correspondente a {inputValue}{" "}
                          </li>
                        ) : (
                          getItmes(inputValue).map((item, index) => (
                            <li
                              className="list-li"
                              {...getItemProps({
                                key: item.id,
          ...
          <div className="col-md-2 col-xs-6 centerDiv">
            ...
            >
              <i className="fas fa-shopping-cart ico" />
              <span className="prodcarte">{this.props.qntCart}</span>
            </a>
          </div>

          <div className="col-md-2 col-xs-6">{this.detectarLogin()}</div>
        </div>
      </div>
    );
  }
